def philia093():
    print('With love as one, cleanse hatred. With me as one, rewrite destruction.')
...
if __name__ == '__main__':
    ...
    for _ in range(n):
        ...
        #上面代码中print(f'\033[0m...\033[0m')是为了控制输出内容的颜色#Python 中的列表底层是一个可以动态扩容的数组，列表元素在计算机内存中是连续存储的，所以可以实现随机访问（通过一个有效的索引获取对应的元素且操作时间与列表元素个数无关）。
    ...
    *i, j = a
    ...
    #原始字符串 Python中的一种以r或R开头的字符串，意思是字符串中的每个字符都是它本来的含义，没有所谓的转义字符。
    r"""s1 = '\it \is \time \to \read \now'  #报错，且无法得到像s2的输出那样的结果"""
    s2 = r'\it \is \time \to \read \now'  #\it \is \time \to \read \now
    print(s1)
    print(s2)#上面的变量s1中，\t、\r和\n都是转义字符。\t是制表符（table），\n是换行符（new line），\r是回车符（carriage return）相当于让输出回到了行首。
    ...
    print(s2 in s1)        # False
       #索引运算和切片运算 运算逻辑和列表、元组没区别，但字符串是不可变类型，所以不能通过索引运算修改字符串中的字符
    s = 'abc123456'
    n = len(s)
    print(s[0], s[-n])    # a a
    print(s[n-1], s[-1])  # 6 6
